...
export const runtime = 'nodejs'; // asegúrate de no usar Edge

// ====== CONFIG ======
const FREE_DAILY_WORDS = 1000;   // Límite gratis / día
const PRO_DAILY_WORDS  = 20000;  // Límite Pro / día (ajústalo)
...
export async function POST(req: NextRequest) {
  // lee el body una sola vez (lo reutilizamos si hay catch)
  ...
  try {
    ...
    if (!apiKey) return jsonError('Falta OPENAI_API_KEY (revisa .env y Vercel)', 500);
    if (!text.trim()) return jsonError('Falta el campo "text" con contenido', 400);

    // ---- LÍMITE DIARIO (Redis/KV duro + cookie visual) ----
